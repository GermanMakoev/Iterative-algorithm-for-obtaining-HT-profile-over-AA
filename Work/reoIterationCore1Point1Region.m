function reoIterationCore1Point1Region(hLib, mfoData, Mmap, freqs, Robs, Lobs, H0, T0, Hb, Tb, Ht1, Ht2, Hc, Tc, NTv, posR, posL, param)
...
title(hAxes(1,2),'Dependence of the deviation on the number of iterations');
xlabel(hAxes(1,2),'Number of iterations');
ylabel(hAxes(1,2),'Deviation');
...
while true    
    ...
    for k = 1:length(freqs)
        if isempty(posL) || isempty(posR)
            [depthRight, pFRight, pTauRight, pHLRight, pFLRight, psLRight, ...
             ...
             pScanRight, pScanLeft] = ...
                     reoCalculate(hLib, mfoData, Hcalc, Tcalc, Dcalc, Mmap, freqs(k), param.harms, param.pTauL, param.mode, param.c, param.b);
             
            [~, posRcalc] = max(pScanRight);
            [~, posLcalc] = max(pScanLeft);
        else
            posRcalc = posR(k);
            posLcalc = posL(k);
        end
        ...
        Rcalc(k) = max(pScanRight);
        Lcalc(k) = max(pScanLeft);
