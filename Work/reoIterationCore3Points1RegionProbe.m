function reoIterationCore3Points1RegionProbe(hLib, mfoData, Mmap, freqs, Robs, Lobs, H0, T0, Hb, Tb, Ht1, Ht2, Hc, Tc, NTv, posR, posL, param)
...
while true    
    ...
    posRcalc = zeros(3, length(freqs));
    posLcalc = zeros(3, length(freqs));
    l=1;
    for k = 1:length(freqs)
        %if isempty(posL) || isempty(posR)
            ...
            %posRcalc(l,k) = 24+14*(l-2);
           % posLcalc(l,k) = 24+14*(l-2);
        %else
            posRcalc(l,k) = posR(l,k);
            posLcalc(l,k) = posL(l,k);
        %end
        ...
        Rcalc(l,k) = pScanRight(24+14*(l-2));
        Lcalc(l,k) = pScanLeft(24+14*(l-2));
